function [PR,RE,AR,RI,MI,HI]=Pre_Rec(c1,c2)
%PRE_REC Summary of this function goes here
%运用validindex函数计算精确率Precision,召回率Recall。
...
PR=(t2-sum(sum(C)))/(njs-sum(sum(C)));
RE=(t2-sum(sum(C)))/(nis-sum(sum(C)));
